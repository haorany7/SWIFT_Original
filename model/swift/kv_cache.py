...
def initialize_past_key_values(model):
    ...
    batch_size = 1
    # Determine the effective max position embeddings
    effective_max_position_embeddings = min(4096, config.max_position_embeddings)
    ...
    for id, i in enumerate(devices):
        if startdevice != i:
            past_key_values_data = torch.zeros(
                ...
                effective_max_position_embeddings,  # Use the effective max position embeddings
